...
function entryPoint(options) {
	...
	case "builds/courses/course":
		options.outputDest = defaults.multipleDestPath;
		break;
	case "src/courses":
		options.outputDest = defaults.multipleDestPath;
		break;
	case "src/course":
		options.outputDest = defaults.singleDestPath;
		break;
	}
...
function buildsFolders(options) {
	fsext.walkSync(fsext.relative(options.outputDest), function(dirs, files) {
		...
		for (var i = 0, l = dirs.length; i < l; i++) {
			...
			var opts = _.extend({}, options, { dest: path.join(options.outputDest, dir), course: dir });
			build(opts);
		}
...
function srcsFolder(options) {
	var opts = _.extend({}, options, { dest: options.outputDest, course: "" });
	build(opts);
}
...
function watchForChanges(options) {
	...
	if (taskqueue.isRunning()) 
		taskqueue.on("end", startServer);
	else startServer();
	function startServer() {
		if (options.switches['server'] && !server.isStarted()) server.start(options);
	}
}
...
function onChange(type, stat, data) {
	...
	case "changed":
		...
		break;
	case "added":
		logger.log(stat.filename+stat.extname + " was added.",1);
		break;
	case "deleted":
		logger.log(stat.filename+stat.extname + " was deleted.",1);
		break;
	}
...
function runAction(options, config) {
	var cloneConfig = _.extend({}, options, config);
