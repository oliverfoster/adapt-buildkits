...
function watchStart() {
	...
	watchingInterval = setInterval(watchLoop, 2000);
...
function watchLoop() {
	...
	for (var k in watching) {
		...
		options.cur = pub.glob(options.path, options.globs, options);
...
function watchChanges(cur, prev, options) {
	...
	for (var d = 0, dl = deleted.length; d < dl; d++) {
		options.callback("deleted", prevIndex[ deleted[d] ], options);
	}

	for (var d = 0, dl = added.length; d < dl; d++) {
		options.callback("added", curIndex [ added[d] ], options);
	}

	for (var d = 0, dl = check.length; d < dl; d++) {
		var item = check[d];
		if (curIndex[item].mtime != curIndex[item].mtime) {
			options.callback("changed", curIndex [ item ], options);
		}
	}
...
var pub = {
	...
	watch: function(options) {
		...
		watching[watchName].cur = pub.glob(options.path, options.globs, options);
